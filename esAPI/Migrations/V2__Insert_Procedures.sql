...
BEGIN
    ...
    IF NOT EXISTS
        (SELECT 1 FROM machine_orders WHERE order_id = p_order_id)
    THEN
        RAISE EXCEPTION 'Order % does not exist', p_order_id;
    END IF;
    
    IF p_output_amount IS NULL OR p_output_amount <= 0 THEN
        RAISE EXCEPTION 'Output amount must be greater than 0';
    END IF;
    ...
    FOR v_key IN SELECT json_object_keys(p_material_ratios) LOOP
        ...
        IF v_ratio IS NULL OR v_ratio <= 0 THEN
            RAISE EXCEPTION 'Ratio for material % must be a positive integer',
                v_key;
        END IF;
        
        SELECT material_id INTO v_material_id FROM raw_materials
        WHERE material_name = v_key;
        ...
        VALUES (v_material_id, v_ratio, v_machine_id);
...
CREATE OR REPLACE PROCEDURE add_material(
    ...
    IN p_price_per_kg NUMERIC(10,2) DEFAULT NULL
) LANGUAGE plpgsql AS $$
BEGIN
    ...
    IF p_price_per_kg IS NOT NULL AND p_price_per_kg < 0.0 THEN
        RAISE EXCEPTION 'Price per kg cannot be a negative value';
    END IF

    INSERT INTO materials (material_name, price_per_kg)
    VALUES (p_material_name, p_price_per_kg);
...
BEGIN
    ...
    IF NOT EXISTS
        (SELECT 1 FROM raw_materials WHERE material_id = p_material_id)
    THEN
...
BEGIN
    ...
    IF p_material_id IS NULL OR NOT EXISTS
        (SELECT 1 FROM raw_materials WHERE material_id = p_material_id)
    THEN
        RAISE EXCEPTION 'Material ID % does not exist', p_material_id;
    END IF;
    ...
    IF p_machine_id IS NULL OR NOT EXISTS
        (SELECT 1 FROM machines WHERE machine_id = p_machine_id)
    THEN
...
BEGIN
    ...
    IF p_supplier_id IS NULL OR NOT EXISTS
        (SELECT 1 FROM companies WHERE company_id = p_supplier_id)
    THEN
        RAISE EXCEPTION 'Company ID % does not exist', p_supplier_id;
    END IF;
    ...
    INSERT INTO machine_orders
        (supplier_id, external_order_id, ordered_at, received_at)
    VALUES
        (p_supplier_id, p_external_order_id, p_ordered_at, p_received_at);
...
BEGIN
    ...
    IF p_material_id IS NULL OR NOT EXISTS
        (SELECT 1 FROM materials WHERE material_id = p_material_id)
    THEN
        RAISE EXCEPTION 'Material ID % does not exist', p_material_id;
    END IF;

    IF p_order_id IS NULL OR NOT EXISTS
        (SELECT 1 FROM material_orders WHERE order_id = p_order_id)
    THEN
...
BEGIN
    ...
    IF p_manufacturer_id IS NULL OR NOT EXISTS
        (
            SELECT 1 FROM phone_manufacturers
    ...
    INSERT INTO electronics_orders
        (manufacturer_id, amount, ordered_at, processed_at)
    VALUES
        (p_manufacturer_id, p_amount, p_ordered_at, p_processed_at);
